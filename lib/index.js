  ...
  fs.readFile(file, function(err, data) {
    ...
    parse(data, {columns: true}, function(err, output) {
      ...
      for (var i = 0; i < output.length; i++) {
        ...
        let record = {
          company: output[i].name,
          companyLink: output[i]['name-href'],
          companyDescription: output[i].description,
          size: output[i].size,
          domain: output[i].domain,
          name: output[i].founderName,
          profileLink: output[i]['founderName-href'],
          bio: output[i].founderBio,
        };
...
start('./angellist-emails-1.csv', function(err, res) {
  if (err) {
    console.log(err);
  }
